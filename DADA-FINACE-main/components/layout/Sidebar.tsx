...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      >
        {/* Logo */}
        <div className="flex items-center justify-between px-5 pt-6 pb-5" style={{ borderBottom: '1px solid #1a1a1a' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-white leading-tight tracking-wide">Dada Finance</div>
              <div className="text-[11px] mt-0.5" style={{ color: '#6B6B6B' }}>Loan Management</div>
            </div>
          </div>
          {onClose && (
            <button
        ...
        <div className="px-4 pt-5 pb-2">
          <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: '#6B6B6B' }}>
            Dashboard
          </p>
          <Link
            href="/"
            ...
            onMouseEnter={e => { if (pathname !== '/') e.currentTarget.style.background = '#141414' }}
            onMouseLeave={e => { if (pathname !== '/') e.currentTarget.style.background = 'transparent' }}
          >
            <LayoutDashboard size={15} style={{ color: pathname === '/' ? '#3b82f6' : '#4b5563' }} />
            <span className="text-sm font-medium">Overview</span>
            {pathname === '/' && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-blue-500" />}
          </Link>
        </div>
          ...
          {NAV.map((item, idx) => {
            ...
            return (
              <div key={item.label}>
                ...
                {idx > 0 && (
                  <div className="mb-4" style={{ borderTop: '1px solid #1a1a1a' }} />
                )}
                ...
                >
                  <span
                    ...
                    style={{ color: active ? '#9ca3af' : '#6B6B6B' }}
                  >
                    {item.label}
                  </span>
                  <ChevronDown
                    size={12}
                    style={{ color: '#6B6B6B', transition: 'transform 0.2s', transform: isOpen ? 'rotate(0deg)' : 'rotate(-90deg)' }}
                  />
                </button>

                {/* Children */}
                <div
                  className="overflow-hidden transition-all duration-200 ease-in-out space-y-0.5"
                    ...
                    return (
                      <Link
                        ...
                        style={{
                          ...
                          background: childActive ? '#1a1a1a' : 'transparent',
                        }}
                        onMouseEnter={e => { if (!childActive) { e.currentTarget.style.background = '#141414'; e.currentTarget.style.color = '#e5e7eb' } }}
                        onMouseLeave={e => { if (!childActive) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#6b7280' } }}
                      >
                        <child.icon
                          size={14}
                          style={{ color: childActive ? '#3b82f6' : '#374151', flexShrink: 0 }}
                        />
                        <span className="text-sm font-medium leading-none">{child.label}</span>
                        {childActive && <span className="ml-auto w-1 h-1 rounded-full bg-blue-500 flex-shrink-0" />}
                      </Link>
                    )
                  })}
                </div>
